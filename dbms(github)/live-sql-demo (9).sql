...
SELECT * FROM employee;
 ...
 FETCH FIRST 1 ROWS ONLY;
  ...
  SELECT id,name,date_of_joining,salary,
    AVG(salary) OVER(ORDER BY department)AS running_AVG
    FROM employee;
...
BEGIN 
    DBMS_OUTPUT.PUT_LINE('HELLO WORLD');
     DBMS_OUTPUT.PUT_LINE('DBMS');
      DBMS_OUTPUT.PUT_LINE('HELLO MOUNI');
    END;
/
DECLARE
--varible declaration-->variablename,datatype(reverse lo vastadi)
    a NUMBER;
    b NUMBER;
    c NUMBER;
BEGIN
...
BEGIN
    ...
    a := 10;
          ...
          areaofsquare :=a*a;
          perimeter :=4*a;
   DBMS_OUTPUT.PUT_LINE('areaofsquare is:'||areaofsquare);
...
BEGIN 
     ...
     ELSE 
      sot :=b;
    END IF;
     DBMS_OUTPUT.PUT_LINE('smaller  of'|| a ||'and' || b|| 'and' ||sot);
    END;
    /
...
BEGIN 
  ...
  --single row function
     ...
     ELSE 
      max_of_three :=C;
    END IF;
     DBMS_OUTPUT.PUT_LINE('largest no is'|| a || ','|| b|| ','||c || 'is'|| max_of_three  );
END;
 /
