...
FROM employee
WHERE department='HR' AND  salary=(SELECT MAX(salary) FROM employee
WHERE department='HR');

SELECT name,salary
FROM employee
WHERE department='Testing' AND salary=(SELECT MAX(salary) FROM employee
WHERE department='Testing');


SELECT name,salary,department
FROM employee
WHERE department='SW' AND salary=(SELECT MAX(salary) FROM employee
WHERE department='SW');


SELECT name,salary,department
FROM employee
WHERE department='Operations' AND salary=(SELECT MAX(salary) FROM employee
WHERE department='Operations');

---MCMR subquery (multiple coloums and multiple rows)
---corelated subquery  -----windows functions(RANK())
...
--who is the youngest employeein their department.....
SELECT id,name,age,department
FROM employee
WHERE (department,age) IN
(SELECT department,MIN(age) AS youngest_person
FROM employee 
WHERE department IS NOT NULL
GROUP BY department);
---co related subquery
--a subquery which depends on one or more coloumns from outer query

--mcmr  (SELECT id,name,salary,department
